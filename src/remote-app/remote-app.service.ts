import { Injectable, Logger, HttpService, OnApplicationShutdown } from '@nestjs/common';
import { interpret } from 'xstate';
import { Interval } from '@nestjs/schedule';
import { APIContainersResponse, APIContainerResponse, ContainerType, ContainerAction, ContainerState, ContainerStateMachine, ContainerContext, Error, ContainerService, ContainerOptions } from './remote-app.types'
import { createContainerMachine, invokeRemoteContainer } from './remote-app.container-machine'
...
export class RemoteAppService implements OnApplicationShutdown {
  ...
  private restoreState = ({ containers }: { containers?: ContainerContext[] }) => {
    // // this.logger.log(JSON.stringify({ containers }), 'restoreMachines')
    if (containers) {
      this.containerServices = containers.map(server => {
 ...
 */
  ...
  private handleTransitionFor = (service: ContainerService) => {
    service.onTransition(state => {
      ...
      if (state.changed) {
        this.logger.log(`${service.state.context.name} ${service.state.context.id} ${state.value}`, 'onTransition')

        if (state.value === ContainerState.DESTROYED) {
          this.removeService(service);
        } else {
          const containers: ContainerContext[] = this.containerServices
            .map(s => ({ ...s.state.context, state: state.value }))
  ...
  private removeService = (service: ContainerService) => {
    const servicesToRemove = this.containerServices.filter(s => s.machine.id === service.machine.id || s.state.context.parentId === service.machine.id) || []
    servicesToRemove.forEach(s => s.stop())

    const nextServices = this.containerServices.filter(s => s.machine.id !== service.machine.id && s.state.context.parentId !== service.machine.id) || []
    this.containerServices = nextServices;
    const containers = nextServices.map(s => ({ ...s.state.context, state: s.state.value }))
  ...
  */
  ...
  pollRemoteState() {
    ...
    services?.forEach(async service => {
      try {
        const nextContext = await invokeRemoteContainer(service.state.context, { type: ContainerAction.STATUS })
        // this.logger.debug(`${JSON.stringify({ nextContext })}`, 'pollRemoteState');
        this.logger.debug(`${service.state.context.name} - ${service.machine.id} | local: ${service.state.value}, remote: ${nextContext.state}, nextAction: ${service.state.context.nextAction}`, 'pollRemoteState');

        if (nextContext.error) {
          // this.logger.debug(ContainerAction.SYNC_STOPPED, 'pollRemoteState');
          service.send({ type: ContainerAction.REMOTE_STOPPED, data: nextContext });
        ...
        switch (nextContext.state) {
          ...
          case (ContainerState.UNINITIALIZED):
            if (service.state.context.nextAction === ContainerAction.DESTROY) {
              ...
              const childApps = this.containerServices.filter(s => s.state.context.parentId === service.machine.id);
            ...
            service.send({ type: ContainerAction.REMOTE_STOPPED, data: { ...nextContext, error: { message: "Container is not reachable" } } });
            break;

          case (ContainerState.RUNNING):
            // this.logger.debug(ContainerAction.SYNC_STARTED, 'pollRemoteState');
            service.send({ type: ContainerAction.REMOTE_STARTED, data: nextContext, error: undefined });
            break;

          case (ContainerState.CREATED):
            service.send({ type: ContainerAction.REMOTE_CREATED, data: nextContext, error: undefined });
            break;
        }

      } catch (error) {
        this.logger.debug(`${service.machine.id} | local: ${service.state.value}, ${JSON.stringify(error)}`, 'pollRemoteState');
        //this.logger.debug(ContainerAction.SYNC_STOPPED, 'pollRemoteState');
  ...
  async startServer(id: string): Promise<APIContainerResponse> {
    ...
    const context: ContainerContext = { id, name, user: USER.hipuser, url: '', state: ContainerState.UNINITIALIZED, error: null, type: ContainerType.SERVER }
    ...
    const nextContext: ContainerContext = { ...service.state.context, state: service.state.value };
  ...
  async startServerWithWebdavCredentials(id: string, login: string, password: string): Promise<APIContainerResponse> {
    // this.logger.log(id, 'startServer');
    ...
    const sessionNames = sessionNamesArray.length > 0 ? sessionNamesArray : [0]
    const name = `${Math.max(...sessionNames) + 1}`
    const context: ContainerContext = { id, name, user: login, password, url: '', state: ContainerState.UNINITIALIZED, error: null, type: ContainerType.SERVER }
    const serverMachine = createContainerMachine(context)
    service = interpret(serverMachine).start();
    this.handleTransitionFor(service)
    // this.logger.log(`${id} ${ContainerAction.START}`, 'startServer');
  ...
  async destroyServer(id: string): Promise<APIContainerResponse> {
    ...
    const appServices = this.containerServices.filter(s => s.state.context.parentId === service.machine.id);
    if (service) {
      if (service.state.value === ContainerState.EXITED) {
        ...
        appServices.forEach(s => {
          s.send({ type: ContainerAction.STOP, data: { ...s.state.context, nextAction: ContainerAction.DESTROY } });
        })
        service.send({ type: ContainerAction.STOP, data: { ...service.state.context, nextAction: ContainerAction.DESTROY } });
      }
  ...
  async startApp(serverId: string, appId: string, appName: string): Promise<APIContainerResponse> {
    // this.logger.log(serverId, 'startApp');

    // check existing server
    let serverService = this.containerServices.find(s => s.machine.id === serverId);
    if (!serverService) {
    ...
    const nextContext: ContainerContext = { ...appService.state.context, state: appService.state.value };
